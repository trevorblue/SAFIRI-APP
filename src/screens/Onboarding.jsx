...
export default function Onboarding({ onComplete, onCancel }) {
  ...
  function set(key, val) {
    setForm(f => {
      if (key === 'groupSize') {
        const names = Array.from({ length: val }, (_, i) => f.memberNames?.[i] ?? '')
  ...
  const canContinue = [
    ...
    () => form.startDate && form.endDate && tripDays > 0,
    () => form.budgetPerPerson > 0,
  ][step]?.()
  ...
  return (
    <div className="fixed inset-0 bg-[var(--color-bg)] flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 pt-14 pb-4">
        ...
        <span className="text-[var(--color-muted)] text-xs tabular-nums w-10 text-right">
          {step + 1}/{STEPS}
        </span>
      </div>
      ...
      <div className="flex-1 relative overflow-hidden">
        <AnimatePresence custom={dir} mode="wait" initial={false}>
          ...
          >
            ...
            {step === 2 && (
              <Step3 form={form} set={set} tripDays={tripDays} dailyBudget={dailyBudget} groupSize={form.groupSize} />
            )}
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      <div className="px-5 pb-12 pt-4">
        <motion.button
          onClick={step < STEPS - 1 ? next : finish}
          disabled={!canContinue}
          className="w-full py-4 rounded-2xl font-semibold text-base flex items-center justify-center gap-2 transition-opacity"
          style={{
            backgroundColor: canContinue ? 'var(--color-primary)' : 'var(--color-surface-3)',
            color: canContinue ? 'var(--color-bg)' : 'var(--color-muted)',
          }}
          whileTap={canContinue ? { scale: 0.96 } : {}}
          transition={{ type: 'spring', stiffness: 500, damping: 25 }}
        >
          {step < STEPS - 1 ? (
            <>Continue <ForwardIcon size={18} stroke="currentColor" /></>
          ) : (
...
function Step1({ form, set }) {
  return (
    <div>
      ...
      >
        <p className="text-[var(--color-primary)] text-xs uppercase tracking-widest font-semibold mb-2">
          Step 1
        </p>
      ...
      >
        <Field label="Trip name" icon={<span className="text-base">✈️</span>}>
          <input
            ...
            placeholder="e.g. Mombasa Weekend"
  ...
  return (
    <div>
      ...
      >
        ...
        <p className="text-[var(--color-muted)] text-sm mb-8">
          Set your dates and the daily budget works itself out.
        </p>
      </motion.div>
      ...
      >
        ...
        <Field label="Pre-trip departure (optional)" icon={<span className="text-base">🚂</span>}>
          <input
            type="date"
            value={form.preTripDate}
            onChange={e => set('preTripDate', e.target.value)}
            className="input-field"
          />
          <p className="text-[var(--color-muted)] text-xs mt-1">
            Expenses before trip start date get tagged as pre-trip
          </p>
        </Field>
        ...
        {tripDays > 0 && (
          ...
          >
            ...
            {dailyBudget > 0 && (
              <Chip label="Daily budget" value={formatKES(dailyBudget)} accent />
            )}
          </motion.div>
        )}
      </motion.div>
...
function Step3({ form, set, tripDays, _dailyBudget, groupSize }) {
  ...
  const total = isTotal ? budgetNum : budgetNum * groupSize
  ...
  return (
    <div>
      ...
      >
        ...
        <p className="text-[var(--color-muted)] text-sm mb-8">
          How many people, what's the budget, how are you getting there.
        </p>
      </motion.div>
      ...
      >
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            <motion.button
              ...
              className="w-11 h-11 rounded-2xl bg-[var(--color-surface-2)] border border-[var(--color-border)] text-[var(--color-text)] text-xl font-light flex items-center justify-center"
              whileTap={{ scale: 0.88 }}
              transition={{ type: 'spring', stiffness: 500, damping: 25 }}
            >
              −
            ...
            <div className="flex-1 text-center">
              <motion.span
                key={form.groupSize}
                className="text-4xl font-bold text-[var(--color-text)] block"
            ...
            <motion.button
              onClick={() => set('groupSize', Math.min(20, form.groupSize + 1))}
              className="w-11 h-11 rounded-2xl bg-[var(--color-primary)] text-[var(--color-bg)] text-xl font-light flex items-center justify-center"
              whileTap={{ scale: 0.88 }}
              transition={{ type: 'spring', stiffness: 500, damping: 25 }}
            >
              +
        ...
        {/* Budget with per-person / total toggle */}
        <div>
          <div className="flex items-center justify-between mb-3">
            <label className="flex items-center gap-1.5 text-[var(--color-muted-2)] text-xs font-medium uppercase tracking-wide">
              <WalletIcon size={16} stroke="var(--color-primary)" />
              Budget (KES)
            </label>
            {/* Toggle */}
            <div className="flex rounded-xl overflow-hidden border border-[var(--color-border)] text-xs">
              {['perPerson', 'total'].map(mode => (
                <motion.button
                  ...
                  animate={{
                    backgroundColor: form.budgetMode === mode ? 'var(--color-primary)' : 'var(--color-surface-2)',
                    color: form.budgetMode === mode ? 'var(--color-bg)' : 'var(--color-muted)',
                  }}
          ...
          <input
            ...
            placeholder={isTotal ? `e.g. ${25000 * form.groupSize}` : 'e.g. 25000'}
            className="input-field"
            inputMode="numeric"
          />
          ...
          {budgetNum > 0 && (
            ...
            >
              {isTotal ? (
                <Chip label="Per person" value={formatKES(perPerson)} accent />
              ) : (
                <Chip label="Group total" value={formatKES(total)} />
              )}
              {dailyPerPerson > 0 && (
                <Chip label="Per day" value={formatKES(dailyPerPerson)} accent={!isTotal} />
              )}
            </motion.div>
          )}
        </div>
        ...
        <Field
          label="Your monthly budget (optional)"
          icon={<span className="text-base">📅</span>}
          hint="Shows trip as % of your month"
        >
          <input
            type="number"
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <TransportOption
              active={form.transportMode === 'sgr'}
              onClick={() => set('transportMode', 'sgr')}
              icon={<TrainIcon size={20} stroke="currentColor" />}
              label="SGR"
              detail={form.sgrCostPerPerson ? `KES ${Number(form.sgrCostPerPerson).toLocaleString()}/person` : 'Set cost below'}
            />
            <TransportOption
              active={form.transportMode === 'car'}
              onClick={() => set('transportMode', 'car')}
              icon={<CarIcon size={20} stroke="currentColor" />}
              label="Car rental"
          ...
          <AnimatePresence mode="wait">
            {form.transportMode === 'sgr' && (
              ...
              >
                <Field
                  label="SGR cost per person (KES)"
                  icon={<TrainIcon size={14} stroke="var(--color-primary)" />}
                  hint="Round trip: 2,000 · One way: 1,000"
                >
                  <input
                    type="number"
                    value={form.sgrCostPerPerson}
                    onChange={e => set('sgrCostPerPerson', e.target.value)}
...
function Chip({ label, value, accent }) {
  return (
    <div className={`flex-1 rounded-xl px-3 py-2 text-center ${
      accent
        ? 'bg-[var(--color-primary-dim)] border border-[color:var(--color-primary)]/20'
        : 'bg-[var(--color-surface-2)] border border-[var(--color-border)]'
    }`}>
      <p className="text-[var(--color-muted)] text-[10px] uppercase tracking-wide">{label}</p>
      <p className={`font-bold text-sm ${accent ? 'text-[var(--color-primary)]' : 'text-[var(--color-text)]'}`}>
        {value}
      </p>
    </div>
  )
  ...
  return (
    <motion.button
      ...
      className={`flex flex-col items-center gap-2 py-4 px-3 rounded-2xl border text-center transition-colors ${
        active
          ? 'bg-[var(--color-primary-dim)] border-[color:var(--color-primary)]/40 text-[var(--color-primary)]'
